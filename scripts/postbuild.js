...
const routes = [
  {
    src: "src/api/admin/clinics/[id]/pharmacies/[pharmacyId]/test/route.ts",
    dst: ".medusa/server/src/api/admin/clinics/[id]/pharmacies/[pharmacyId]/test/route.js",
    content: `"use strict";
...
async function PATCH(req, res) {
  try {
    ...
    const { status, is_automatic, discount_value, application_method_id, ends_at, usage_limit } = req.body || {};
    const promoUpdates = [];
    const promoValues = [];
    ...
    if (promoUpdates.length > 1) {
      await pg.raw(\`UPDATE promotion SET \${promoUpdates.join(", ")} WHERE id = ?\`, [...promoValues, promotionId]);
    }
    if (discount_value !== undefined && application_method_id) {
      ...
      await pg.raw("UPDATE promotion_application_method SET value = ?, raw_value = ?::jsonb, updated_at = NOW() WHERE id = ?", [numVal, rawVal, application_method_id]);
    }
    if (ends_at !== undefined) {
      const campaignResult = await pg.raw("SELECT campaign_id FROM promotion WHERE id = ? LIMIT 1", [promotionId]);
      const campaignId = campaignResult.rows[0]?.campaign_id;
      if (campaignId) {
        await pg.raw("UPDATE promotion_campaign SET ends_at = ?, updated_at = NOW() WHERE id = ?",
          [ends_at ? new Date(ends_at).toISOString() : null, campaignId]);
        if (usage_limit !== undefined) {
          await pg.raw(\`UPDATE promotion_campaign_budget SET "limit" = ?, updated_at = NOW() WHERE campaign_id = ? AND type = 'usage'\`,
